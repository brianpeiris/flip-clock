...
const cache = new Map();
function getCanvases(noiseImageData, brushedImageData, digit, flipped) {
  ...
  if (!cache.has(key)) {
    ...
    drawDigit(mapCtx, digit, "black", "white");
...
export function addDigitTextures(noiseImageData, brushedImageData, plane, digit, flipped) {
  const canvases = getCanvases(noiseImageData, brushedImageData, digit, flipped);
